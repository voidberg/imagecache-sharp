...
const PluginScaleCrop: Plugin = {
  ...
  description: '',
  actions: {
    scale_and_crop: (instance: ImageCache, image: sharp.Sharp, metadata, config): Promise<sharp.Sharp> => {
      const convertDimension = instance.getAction('convertDimension');
      const width = config.width ? convertDimension(instance, config.width, metadata.width) : metadata.width;
      const height = config.height ? convertDimension(instance, config.height, metadata.height) : metadata.height;
