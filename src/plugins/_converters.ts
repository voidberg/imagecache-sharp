...
const PluginConverter: Plugin = {
  ...
  actions: {
    convertDimension: (instance: ImageCache, value: string | number, maxDimension: number): number => {
      let converted: number;

      if (S(value).endsWith('%')) {
        converted = ~~((maxDimension * S(value).replaceAll('%', '').toInt()) / 100);
      } else {
    ...
    convertPosition: (instance: ImageCache, value: string | number, maxPosition: number, imageSize: number): number => {
      let converted: number;
