...
const PluginScale: Plugin = {
  ...
  description: '',
  actions: {
    scale: (instance: ImageCache, image: sharp.Sharp, metadata, config): Promise<sharp.Sharp> => {
      ...
      const maxWidth = config.maxWidth ? convertDimension(config.maxWidth, iWidth) : 0;
      const maxHeight = config.maxHeight ? convertDimension(config.maxHeight, iHeight) : 0;
