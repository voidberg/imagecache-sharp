...
describe("ImageCache", () => {
  it("loads all plugins", () => {
    ...
    assert.deepStrictEqual(plugins, expectedPlugins);
  });
  ...
  it("loads presets", () => {
    ...
    assert.deepStrictEqual(presets, ["s_crop_small", "s_crop_teaser"]);
  });
  ...
  it("fails when using an undefined preset", (done) => {
    ...
    imagecache.render(resolve(__dirname, "./in.png"), "foo_bar", (err) => {
      ...
      assert.strictEqual(err.message, "Preset foo_bar could not be found.");
  ...
  it("fails when using an undefined action", (done) => {
    ...
    imagecache.render(resolve(__dirname, "./in.png"), "s_crop_small", (err) => {
      ...
      assert.strictEqual(
        err.message,
        "Action unknown_action for preset s_crop_small not found in loaded plugins."
      );
